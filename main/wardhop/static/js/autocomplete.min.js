function autocomplete(inp,arr){arr=deserializeArray(arr);var currentFocus;inp.addEventListener("input",function(e){var a,b,i,val=this.value;closeAllLists();if(!val){return false;}
...
function deserializeArray(arr){var deseralizedArray=[];split=arr.split(", ")
for(var i=0;i<split.length;i++){split[i]=split[i].replaceAll("\"","").replaceAll("[","").replaceAll("]","")
deseralizedArray.push(split[i]);}
return deseralizedArray;}
document.addEventListener("click",function(e){closeAllLists(e.target);});}
var champions=JSON.parse(document.getElementById('champions-data').textContent);autocomplete(document.getElementById("Champion"),champions);
