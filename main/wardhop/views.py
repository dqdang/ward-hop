...
class PickBanView(View):
    ...
    def get(self, request):
        lobby = request.session["lobby"]
        return render(request, self.draft_template, {"lobby": lobby, "blue_ban": blue_ban, "red_ban": red_ban, "blue1": blue[0], "red1": red[0], "blue2": blue[1], "red2": red[1], "blue3": blue[2], "red3": red[2], "blue4": blue[3], "red4": red[3], "blue5": blue[4], "red5": red[4], "error": ""})

    def post(self, request):
        all_images = [settings.STATIC_URL + self.images[champ]["full"]
                      for champ in self.images.keys()]
        lobby = request.session["lobby"]
        blue_ban = request.session["blue_ban"]
        red_ban = request.session["red_ban"]
        ...
        if champ:
            ...
            if len(found) > 0:
                ...
                return render(request, self.draft_template, {"lobby": lobby, "blue_ban": blue_ban, "red_ban": red_ban, "blue1": blue[0], "red1": red[0], "blue2": blue[1], "red2": red[1], "blue3": blue[2], "red3": red[2], "blue4": blue[3], "red4": red[3], "blue5": blue[4], "red5": red[4], "error": error})
        else:
            all_images = [settings.STATIC_URL + self.images[champ]["full"]
                          for champ in self.images.keys()]
            return render(request, self.draft_template, {"lobby": lobby, "blue_ban": blue_ban, "red_ban": red_ban, "blue1": blue[0], "red1": red[0], "blue2": blue[1], "red2": red[1], "blue3": blue[2], "red3": red[2], "blue4": blue[3], "red4": red[3], "blue5": blue[4], "red5": red[4], "error": ""})
    ...
    def clean_cookies(self, request, lobby):
        ...
        request.session["rotation_counter"] = 0
        lobby = hashlib.sha224(lobby.encode('utf-8')).hexdigest()
        request.session["lobby"] = lobby
        request.session["blue_ban"] = ["/static/Placeholder.png", "/static/Placeholder.png",
                                       "/static/Placeholder.png", "/static/Placeholder.png", "/static/Placeholder.png"]
        request.session["red_ban"] = ["/static/Placeholder.png", "/static/Placeholder.png",
